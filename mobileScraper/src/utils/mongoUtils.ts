import { MongoClient, GridFSBucket, ObjectId, ReturnDocument } from "mongodb";
...
export async function uploadToMongoAndDeleteFile(
  username: string,
  fileName: string,
  platform: string
) {
  ...
  try {
    ...
    console.log(`Data uploaded to MongoDB for user: ${username}`);

    // Delete the JSON file
    await fs.unlink(path.join(process.cwd(), fileName));
    console.log("Temporary JSON file deleted");
  } catch (error) {
    console.error("Error during MongoDB upload:", error);
    throw error;
  } finally {
    await client.close();
  }
