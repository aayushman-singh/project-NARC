...
const connectDB = async () => {
  try {
    await mongoose.connect('mongodb+srv://aayushman2702:Lmaoded%4011@cluster0.eivmu.mongodb.net/whatsappDB?retryWrites=true&w=majority', {
...
app.get('/whatsapp/users/:username', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const user: IWhatsappUser | null = await WhatsappUser.findOne({ username }).lean();
