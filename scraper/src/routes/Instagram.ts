...
const connectDB = async () => {
    try {
      await mongoose.connect('mongodb+srv://aayushman2702:Lmaoded%4011@cluster0.eivmu.mongodb.net/instagramDB?retryWrites=true&w=majority', {
...
app.get('/instagram/users', async (req: Request, res: Response) => {
  ...
  app.get('/instagram/users/:username', async (req: Request, res: Response) => {
    ...
    try {
      ...
      const user: IInstagramUser | null = await InstagramUser.findOne({ username }).lean();
